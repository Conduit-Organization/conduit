...
### M4 — Payments, trust & resilience ⬜
**Goal:** make paid answers instant, dependable, and harder to abuse.

**Tasks:**
- [ ] **Prepaid balance (instant payments):** buyer **deposits 1–2 USD₮** to open a paid **session** with a seller (one on-chain tx — the deposit *is* the session grant); each question **draws down off-chain** (signed receipts) → no per-question on-chain wait. **Refund the remainder** on exit; cap deposit at 1–2 USD₮ to bound trust. Relates to x402 / EIP-3009 (see SDK constraints).
- [ ] **Verifier signal** for confident-but-wrong (the 0.6B "RSA = Rabin-Karp" case) → escalate instead of serving wrong-and-free.
- [ ] **Seller reputation** — track success/latency per seller; surface it in the marketplace + bookmarks (§3.5) and feed the "Auto" pick (Binance-P2P-style completion-rate signal).
- [ ] **Pay-first trust risk** (honest open issue): buyer pays before the answer. Mitigations: small per-call / capped deposit, deliver-or-refund window, reputation; escrow/streaming later. v1 stance: testnet + ≤2 USD₮ + reputation.
- [ ] Reconnect, teardown (orphan fix), error states in UI, model-download/offline handling.
